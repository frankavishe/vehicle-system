...
export function CartTable({ initialCart }: { initialCart: Cart }) {
  ...
  if (cart.items.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-line bg-surface-raised p-10 text-center">
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {cart.items.map((item) => (
          <div key={item.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-0.5">
              <Link href={`/catalog/${item.spare_part.id}`} className="text-sm font-semibold text-asphalt hover:text-primary">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <span className="w-28 text-right text-sm font-semibold text-asphalt">
      ...
      <div className="flex items-center justify-between rounded-2xl border border-line bg-surface-raised p-4 shadow-sm">
        <span className="text-sm font-semibold text-steel">Total</span>
        <span className="font-display text-xl font-semibold text-asphalt">{formatTZS(total)}</span>
      </div>
