...
export function PartCard({ part }: { part: SparePart }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-3 rounded-2xl border border-line bg-surface-raised p-3 shadow-sm transition-shadow hover:shadow-md"
    >
      <div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-surface">
        {part.image_url ? (
      ...
      <div className="flex flex-1 flex-col gap-1">
        <h3 className="text-sm font-semibold text-asphalt group-hover:text-primary">{part.title}</h3>
        <p className="text-xs text-steel-soft">{part.vendor?.name ?? "Unlisted vendor"}</p>
      </div>

      <div className="flex items-center justify-between">
        <span className="font-display text-sm font-semibold text-asphalt">{formatTZS(part.price)}</span>
