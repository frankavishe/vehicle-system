...
export default async function OrdersPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="font-display text-3xl font-bold text-asphalt">My orders</h1>
        ...
        <div className="flex flex-col divide-y divide-line rounded-2xl border border-line bg-surface-raised shadow-sm">
          {orders.map((order) => (
            <Link
              ...
              className="flex flex-col gap-2 p-4 first:rounded-t-2xl last:rounded-b-2xl hover:bg-surface sm:flex-row sm:items-center sm:justify-between"
            >
              <div className="flex flex-col gap-0.5">
                <span className="text-sm text-asphalt">Order #{order.id.slice(0, 8)}</span>
                <span className="text-xs text-steel-soft">
                  {formatDate(order.created_at)} · {order.items.length} item{order.items.length === 1 ? "" : "s"}
                </span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-asphalt">{formatTZS(order.total_amount)}</span>
