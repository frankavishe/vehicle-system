...
Phase 3's Flutter client — role-based dashboards for Customer, Mechanic,
Recovery Operator, and (thinly, this phase) Admin. See `PLAN.md` §7 Phase 3
and `C:\Users\User\.claude\plans\phase-3-flutter-dispatch-plan.md` for the
full design and every flagged package/scope decision.
...
and the dio 401-refresh-retry interceptor
(`dio_401_retry_test.dart`, against a scripted fake `HttpClientAdapter` —
no real network, no mocking package). No widget/golden/integration suite
this phase.
